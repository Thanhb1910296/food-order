...
class MerchantServices {
  void sellProduct({
    ...
    try {
      final cloudinary = CloudinaryPublic('de4wk7obp', 'vyju1pmt');
      ...
      http.Response res = await http.post(
        Uri.parse('$uri/merchant/add-product'),
        headers: {
      ...
      httpErrorHandle(
        ...
        onSuccess: () {
          ...
          Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (context)=>MerchantScreen()), (route) => false);
        },
      );
    } catch (e) {
  ...
  Future<List<Product>> fetchAllProducts(BuildContext context) async {
    ...
    try {
      http.Response res =
          await http.get(Uri.parse('$uri/merchant/get-products'), headers: {
        'Content-Type': 'application/json; charset=UTF-8',
        'x-auth-token': userProvider.user.token,
      });
  ...
  void deleteProduct({
    ...
    try {
      http.Response res = await http.post(
        Uri.parse('$uri/merchant/delete-product'),
        headers: {
          'Content-Type': 'application/json; charset=UTF-8',
          'x-auth-token': userProvider.user.token,
        },
  ...
  Future<List<Order>> fetchAllOrders(BuildContext context) async {
    ...
    try {
      http.Response res =
          await http.get(Uri.parse('$uri/merchant/get-orders'), headers: {
        'Content-Type': 'application/json; charset=UTF-8',
        'x-auth-token': userProvider.user.token,
      });
  ...
  void changeOrderStatus({
    ...
    try {
      http.Response res = await http.post(
        Uri.parse('$uri/merchant/change-order-status'),
        headers: {
          'Content-Type': 'application/json; charset=UTF-8',
          'x-auth-token': userProvider.user.token,
        },
  ...
  //   try {
  //     http.Response res =
  //         await http.get(Uri.parse('$uri/merchant/analytics'), headers: {
  //       'Content-Type': 'application/json; charset=UTF-8',
  //       'x-auth-token': userProvider.user.token,
  //     });
  //
